<template>
  <div class="star" :class="starType">
    <span v-for="(item, index) in itemClass" :class="item" :keys="index" class="star-item"></span>
  </div>
</template>
<script>
  const LENGTH = 5;
  ...
  export default {
    ...
    computed: {
      ...
      itemClass() {
        ...
        let integer = Math.floor(score);//整书
        ...
        while (result.length < LENGTH) { // 是否有小数，只判断一次因为只可能有一颗半星
          result.push(CLS_OFF)
        }
//        console.log(result);
        return result
